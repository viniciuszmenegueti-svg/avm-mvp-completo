...
@pytest.mark.parametrize(
    "environment",
    ["homologation", "staging", "unknown-environment"],
)
def test_client_auth_fails_closed_outside_known_development_environments(
    monkeypatch: pytest.MonkeyPatch,
    environment: str,
) -> None:
    monkeypatch.setattr(client_auth, "APP_ENV", environment)
    monkeypatch.setattr(
        client_auth,
        "CLIENT_CREDENTIALS_JSON",
        '{"caixa":"012345678901234567890123"}',
